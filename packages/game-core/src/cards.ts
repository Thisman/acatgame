...
export type CardAnimationVariant = 'default' | 'blocker';
...
export type CardMechanicDefinition = PlacementLockAuraMechanicDefinition;
...
export type BoardCellEffect = PlacementLockEffect;
...
const CARD_DEFINITIONS = Array.from({ length: READY_CARD_POOL_SIZE }, (_value, cardID) =>
  BLOCKER_CARD_IDS.has(cardID) ? createBlockerCardDefinition(cardID) : createNormalCardDefinition(cardID),
);
